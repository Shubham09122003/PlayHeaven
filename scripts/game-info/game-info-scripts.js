...
function loadGameInfo(gameCode) {
  ...
  priceTag.innerHTML = `<i class='bi bi-cart-plus-fill pe-2'></i>Buy - $${selectedGame.price}`;
...
function loadCategoryElement(category) {
  switch (category) {
    case 'shooter':
      return `<p class='badge bg-danger fs-6'><i class='bi bi-bullseye pe-2'></i>Shots</p>`

    case 'puzzle':
      return `<p class='badge bg-warning text-black fs-6'><i class='bi bi-puzzle pe-2 text-black'></i>Puzzle</p>`

    case 'strategy':
      return `<p class='badge bg-info text-black fs-6'><i class='bi bi-pie-chart pe-2 text-black'></i>Strategy</p>`

    default:
      return `<p class='badge bg-dark fs-6'><i class='bi bi-controller pe-2'></i>Other</p>`;
  }
}
